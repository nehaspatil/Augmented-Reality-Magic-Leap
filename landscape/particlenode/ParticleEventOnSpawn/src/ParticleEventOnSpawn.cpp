...
#include <ParticleEventOnSpawn.h>
...
void ParticleEventOnSpawn::onAttachPrism(lumin::Prism* prism)
{
	m_currentPrism = prism;

	auto packageRes = prism->createParticlePackageResourceId("res/particle_noSpawnDeath");
	auto m_ParticleNode1 = prism->createParticleNode(packageRes);
	m_ParticleNode1->setEffectName("Particles/Audio/AudioRendererTest.pkfx");
	m_ParticleNode1->setLocalScale(glm::vec3(.01f, .01f, .01f));
	m_ParticleNode1->setLocalPosition(glm::vec3(0, -0.9f, 0));
...
bool ParticleEventOnSpawn::onEvent(lumin::ServerEvent* a_pEvent)
{	
...
void ParticleEventOnSpawn::CreateTextandButtonNodes()
{
	m_titleNode = lumin::ui::UiText::Create(m_currentPrism, "OnSpawn Spawn and Death ");
