...
particlenode::particlenode() {
  ML_LOG(Debug, "ParticleSamplerBox Constructor.");  
}

particlenode::~particlenode() {
  ML_LOG(Debug, "ParticleSamplerBox Destructor.");

}

int particlenode::init() {
  ML_LOG(Debug, "ParticleSamplerBox Initializing.");  

  return 0;
}

void particlenode::onAppStart(const InitArg&) {
  ML_LOG(Debug, "ParticleSamplerBox creating instance");
  // Create a prism
  glm::vec3 size(2.0f, 2.0f, 2.0f);
  Prism* prism = requestNewPrism(size);
  if (!prism) {
    ML_LOG(Error, "ParticleSamplerBox Error creating default prism.");
  }
  // Cursor size is determined from prism size. Set the scale to ignore the default size.
  ui::Cursor::SetScale(prism, 0.03f);

  auto CurrentPrism = std::make_shared<ParticleSamplerBox>();
  prism->setPrismController(CurrentPrism);
}

int particlenode::deInit() {
  ML_LOG(Debug, "ParticleSamplerBox Deinitializing.");
